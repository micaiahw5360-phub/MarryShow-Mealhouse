...
import { itemsService } from '../services/api';  // ADDED
...
export function Menu() {
  ...
  const filteredItems = items.filter((item) => {    // CHANGED: was menuItems.filter
  ...
  const handleAddToCart = () => {
    ...
    addItem(selectedItem, 1, {});   // CHANGED: was addItem(selectedItem, options)
  ...
  if (loading) return <div className="text-center py-20">Loading menu...</div>;   // ADDED
                  ...
                  <div className="relative aspect-square overflow-hidden bg-gray-100">
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        toast.success(isFavorite(item.id) ? 'Removed from favorites' : 'Added to favorites!');
                      }}
                      className="absolute top-3 right-3 w-9 h-9 rounded-full bg-white shadow-md flex items-center justify-center hover:bg-gray-50 transition-colors z-10"
                    >
                      <Heart
                        className={`h-4 w-4 ${isFavorite(item.id) ? 'fill-red-500 text-red-500' : 'text-gray-600'}`}
                      />
                    </button>
                  </div>
